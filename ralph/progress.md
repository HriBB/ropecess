...
PRD: #2. Slices: #3–#11 (`ready-for-agent`), #12 is HITL (`ready-for-human` — not ours). Bugfixes: #13, #14.

---

## 2026-06-06 — Issue #14: Card images — missing `sizes` on `<source>` elements

**Branch:** `fix/14-card-image-sizes` — **PR:** https://github.com/HriBB/ropecess/pull/16

**Built:** One-file fix in `app/components/Picture.tsx`. The `sizes` prop was forwarded to `<img>` via `{...props}` but never to `<source>` elements. Per the HTML spec, `<source srcset>` with width descriptors (`400w`/`600w`) uses the `sizes` attribute on the `<source>` to select a candidate; without it the browser defaults to `100vw` and always downloads the 600 w image for small card slots. Extracting `sizes` explicitly and passing it to every `<source sizes={sizes}>` and `<img sizes={sizes}>` fixes the selection for all five Design Variants — no call-site changes needed since every `<Picture>` already carries a correct `sizes` prop.

**Test/build results:** 26 unit tests pass (4 files). `pnpm typecheck`, `pnpm lint`, `pnpm build` all clean. Build asset sizes: `industrial-rope-access-01` 400 w AVIF = 45.91 kB, 600 w AVIF = 102.84 kB. Before fix: browser picks 600 w (102.84 kB) for 80 px card slots. After: picks 400 w (45.91 kB). Savings: −56.93 kB per card × 5 = −285 kB per page (matches Lighthouse estimate of 297 kB on `/a/`, 216 kB on `/e/`).

**Deferred/surprising:** No more `ready-for-agent` issues remain. All `?thumb`/`?square` presets were already in place across all content modules from S3; this was purely a `Picture` component propagation gap. The 331 kB JPEG for `industrial-rope-access-01` in the build output appears to be generated by `vite-plugin-lqip` (its `src` field) — not related to this fix.
