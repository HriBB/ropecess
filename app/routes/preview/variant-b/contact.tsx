...
export default function VariantBContact() {
      ...
      <section className="px-6 pb-32 md:px-12">
        <div className="grid gap-16 lg:grid-cols-2">
          <div>
            <h2
              style={garamond}
              className="text-5xl font-medium leading-tight mb-8 md:text-7xl"
            >
              {d.hero.title.split(' ').map((w, i, arr) => (
                <span
                  key={i}
          ...
          >
            ...
            <div className="flex flex-col gap-2">
              <input
                ...
                className="border-b border-base-content/30 bg-transparent pb-3 text-2xl outline-none transition-colors focus:border-base-content"
                style={garamond}
                autoComplete="name"
                disabled={recaptcha.isBusy}
              />
              {errors?.name && (
                <p className="text-xs text-error" style={lato}>{errors.name}</p>
              )}
            </div>
            <div className="flex flex-col gap-2">
              <input
              ...
              />
              {errors?.email && (
                <p className="text-xs text-error" style={lato}>{errors.email}</p>
              )}
            </div>
            <div className="flex flex-col gap-2">
              <textarea
                ...
                rows={4}
                className="resize-none border-b border-base-content/30 bg-transparent pb-3 text-2xl outline-none transition-colors focus:border-base-content"
                style={garamond}
                disabled={recaptcha.isBusy}
              />
              {errors?.message && (
                <p className="text-xs text-error" style={lato}>{errors.message}</p>
              )}
            </div>
            <button
              type="submit"
